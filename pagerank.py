...
def transition_model(corpus, page, damping_factor):
    ...
    for p in corpus:
        ...
        # Divide 1 - damping_factor among all pages
        page_rank = (1 - damping_factor) / number_of_pages

        # If no connections, add eq probability
        if len(corpus[page]):
            if p in corpus[page]:
                page_rank += damping_factor / number_of_pages
        model[p] = page_rank
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    if corpus:
        ...
        for _ in range(n):
            ...
            curr_page = random.choice(list(model.keys()))
